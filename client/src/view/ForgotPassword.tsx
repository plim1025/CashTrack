...
// UTILS //
import { validateEmail } from '../components/shared/SharedUtils';
...
const ForgotPassword: React.FC<RouteComponentProps> = props => {
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLElement>) => {
        ...
        if (!validateEmail(email)) {
            setError({ show: true, message: 'Please enter a valid email', status: 'danger' });
        } else {
            setLoading(true);
            try {
                ...
                setError({ show: true, message: 'Sent reset link to inbox', status: 'success' });
            } catch (err) {
                throw Error(`Error sending forgot password email: ${err}`);
            }
            if (errorTimeout) {
